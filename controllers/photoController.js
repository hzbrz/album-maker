      ...
      .then(snap => {
        return new Promise((resolve, reject) => {
          for (const photo of snap.data().photos) {
            // getting the photos from the photo collection using the ids from the album coll
            firestore.collection("photos").doc(photo.photoId).get()
              .then(snap => {
                // need to get the user's name who took the photo
              ...
              .catch(err => reject("cannot get the photo collection ", err))
          }
        })
        .then(imageArr => {
          // resolved array of the object that I get from promise above
...
exports.deletePhoto = (req, res, next) => {
  ...
  const matchUser = req.params.matchUser;
  let firestore = firebase.firestore();
  ...
  if (userId === matchUser) {
    ...
    firestore.collection("albums").doc(albumId).update({
      photos: firebase.firestore.FieldValue.arrayRemove({ photoId: photoId })
    })
    ...
    res.json({ message: "Cannot delete, photo was not created by the user", path: null })
  }
}
